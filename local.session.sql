-- DROP DATABASE IF EXISTS nowoncampus;

-- create database nowoncampus;
-- use nowoncampus;
...
-- Updated: Added 'created_by' directly here to avoid ALTER error
CREATE TABLE events (
    event_id INT NOT NULL AUTO_INCREMENT,
    event_name VARCHAR(255) NOT NULL,
    start_datetime DATETIME NOT NULL,
...
-- Updated: Added 'created_by' and 'rejected_by' directly here
CREATE TABLE temporary_events (
    event_id INT NOT NULL AUTO_INCREMENT,
    event_name VARCHAR(255) NOT NULL,
    start_datetime DATETIME NOT NULL,
    ...
    PRIMARY KEY (event_id),
    CONSTRAINT fk_temp_events_created_by FOREIGN KEY (created_by) REFERENCES users(id) ON DELETE SET NULL ON UPDATE CASCADE
);
...
CREATE TABLE EventApprovals (
    ...
    request_timestamp TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    response_timestamp TIMESTAMP NULL,
    FOREIGN KEY (temp_event_id) REFERENCES temporary_events(event_id) ON DELETE CASCADE,
